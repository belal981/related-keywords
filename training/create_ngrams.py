...
def get_phrases(filenames):
    ...
    for line in f:
        if "<doc" in line:
            # Just to keep a track that documents are being processed and their count.
            doc_number = doc_number + 1
            # Print number every 500 documents. Since files are multiprocessed count numbers can be repititive.
            if (doc_number % 5000 == 0):
...
for item in list_files:
    ...
    # Getting the full path for subdirectories.
    inp_subdir = args.input_dir + "/" + item
    subfiles = os.listdir(inp_subdir)     # listing the file in subdirectory
    out_subdir = args.output_dir + "/" + item
    # Making new sub directories in output location, so that the directory structure is same as input directoryy
    query = "mkdir " + out_subdir
    os.system(query)

    for elem in subfiles:
        input_file = inp_subdir + "/"+elem
        output_file = out_subdir + "/"+elem+"_ngram"
        # Getting path of all files that needs to be converted in ngrams.
        inp_filenames.append(input_file)
        # The output names with for files with path.
        out_filenames.append(output_file)
